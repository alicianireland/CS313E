...
#  Description: Practicing permutation and recursion.
...
def groupSumClump(start, nums, target):
    if start >= len(nums):
        return target == 0
    elif start + 1 < len(nums) and nums[start + 1] == nums[start] :
        i  = start
        count = 0
        while nums[i] == nums[start] and i < len(nums):
            i += 1
            count += 1
        return groupSumClump(i, nums, target - (nums[start] * count)) or groupSumClump(i, nums, target)
    else: 
...
def splitArray(nums):
    ...
    if len(nums) == 0:
        ...
        sub = []
        return splitArrayHelper (nums, sub, 0)

def splitArrayHelper(nums, sub, lo):
    high = len(nums)
    if lo < high:
        if sum(sub) == (sum(nums) - sum(sub)):
            ...
            c = sub[:]
            sub.append (nums[lo])
            return splitArrayHelper (nums, sub, lo + 1) or splitArrayHelper (nums, c, lo + 1)
    else:
        return False
...
def splitOdd10(nums):
    if sum(nums) % 2 == 0 or len(nums) == 0:
        return False
    else:
        sub = []
        return splitOdd10Helper (nums, sub, 0)

  
def splitOdd10Helper(nums, sub, lo):
    high = len(nums)
    if lo < high:
        if sum(sub) % 10 == 0 and (sum(nums) - sum(sub)) % 2 != 0 :
            return True
        else:
            c = sub[:]
            sub.append (nums[lo])
            return splitOdd10Helper (nums, sub, lo + 1) or splitOdd10Helper (nums, c, lo + 1)
...
def split53(nums):
    if sum(nums) % 2 != 0:
        ...
        sub = []
        return split53Helper (nums, sub, 0)

def split53Helper(nums, sub, lo):
    high = len(nums)
    if lo < high:
        if sum(sub) == (sum(nums) - sum(sub)):
            return True
            ...
            return split53Helper (nums, sub, lo + 1) or split53Helper (nums, c, lo + 1)
