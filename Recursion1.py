...
def array220(nums, index):
    if index >= len(nums):
        return False
    elif 10 * nums[index] in nums[index + 1:]:
        return True
    else:
...
def pairStar(str):
    if len(str) == 0 or len(str) == 1:
        return str
    elif str[:2][0] == str[:2][1] :
        return str[:2][0] + '*' + pairStar(str[1:])
    else:
...
def countHi2(str):
    if len(str) == 0 or len(str) == 1:
        return 0
    elif str.startswith('hi'):
        return 1 + countHi2(str[2:])
    elif str.startswith('xhi'):
...
def nestParen(str):
    if len(str) == 0 or len(str) == 1:
        return False if (str == '(' or str == ')') else True
    elif str[0] == '(':
        if str[-1] == ')':
